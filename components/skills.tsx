  ...
  return (
    <div className={`${containerClassName || 'mb-12 lg:mb-0 lg:inline-flex text-center'}`}>
      {filteredSkills.map((skill,index) => {
        return (
          <i key={skill.name+index} className={`${iconClassName || 'flex-1 hover:text-sajilo-yellow text-gray-700 text-center pl-2 pr-2 pt-4 lg:pt-6 lg:pl-0 lg:pr-4 text-3xl lg:text-6xl'} lab la-${skill.name}`}></i>
        )
      })}
    </div>
  )
